...
class CalcGISVolumeModified:
    ...
    def __init__(self):
        ...
        polygonShapefilePath = CalcGISVolumeModified.INPUT_POLYGON_DIR + "/modelGridChannelPolygons.shp"
        ...
        if repeatPolygonRasterCreation:
            if arcpy.Exists(CalcGISVolumeModified.POLYGON_RASTERS_GDB):
                arcpy.Delete_management(CalcGISVolumeModified.POLYGON_RASTERS_GDB)
            # Execute CreateFileGDB
            if not arcpy.Exists("PolygonRasters.gdb"):
                arcpy.CreateFileGDB_management(CalcGISVolumeModified.GEOPROCESSING_OUTPUT_ROOT_DIR, "PolygonRasters.gdb")
        ...
        for DEMFile in DEMFiles:
            dbfPath = CalcGISVolumeModified.RESULTS_OUTPUT_DIR + "/" + DEMFile + "_DataTable.dbf"
            dbfFile = open(dbfPath, 'wb')
            asciiFilename = CalcGISVolumeModified.RESULTS_OUTPUT_DIR + "/" + DEMFile + "_CutFillResults.csv"
            asciiFile = open(asciiFilename, 'w')
            ...
            if arcpy.Exists(CalcGISVolumeModified.CUT_AND_FILL_GDB):
                arcpy.Delete_management(CalcGISVolumeModified.CUT_AND_FILL_GDB)
            # Execute CreateFileGDB
            # out_folder_path = CalcGISVolumeModified.root_dir
            if not arcpy.Exists("VolumeCalcCutFill.gdb"):
                arcpy.CreateFileGDB_management(CalcGISVolumeModified.GEOPROCESSING_OUTPUT_ROOT_DIR, "VolumeCalcCutFill.gdb")

            # DEMFile = 'dem_ccfb_south_delta_san_joaquin_rvr_2m_201806121.tif'
            DEMPath = CalcGISVolumeModified.DEM_INPUT_DIR + "/" + DEMFile
    ...
    def computeGISVolume(self, repeatPolygonRasterCreation, DEMfile, field_name, polygonShapefilePath, asciiFile, dbfFile):
        ...
        for row in polygonCursor:
            ...
            if repeatPolygonRasterCreation:
                ...
                OutRaster = CalcGISVolumeModified.POLYGON_RASTERS_GDB + '/' + FCname
            ...
            in_before_surface = CalcGISVolumeModified.POLYGON_RASTERS_GDB + '/' + field_name + str(pIndex)
            out_raster= CalcGISVolumeModified.CUT_AND_FILL_GDB + "/CutFill" + str(pIndex)
